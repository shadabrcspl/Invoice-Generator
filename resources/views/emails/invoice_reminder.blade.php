...
<table width="100%" cellpadding="0" cellspacing="0" style="background:#f0f4f8;padding:40px 0;">
  <tr><td align="center">
    <table width="640" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
      <!-- Header -->
      <tr>
        <td style="background:linear-gradient(135deg,#7c2d12,#ea580c);padding:32px 40px;">
          <table width="100%" cellpadding="0" cellspacing="0">
            <tr>
              <td>
                ...
                <p style="margin:6px 0 0;color:rgba(255,255,255,0.8);font-size:13px;">⚠️ Payment Reminder & Follow-up</p>
              </td>
              <td align="right">
      ...
      <!-- Date Row -->
      <tr>
        <td style="padding:20px 40px;background:#fff7ed;border-bottom:1px solid #fed7aa;">
          <table width="100%" cellpadding="0" cellspacing="0">
            <tr>
              <td style="color:#7c2d12;font-size:12px;text-transform:uppercase;">Due Date<br><strong style="color:#c2410c;font-size:15px;text-transform:none;">{{ $invoice->due_date->format('d M Y') }}</strong></td>
              <td align="center" style="color:#7c2d12;font-size:12px;text-transform:uppercase;">Amount Pending<br><strong style="color:#c2410c;font-size:15px;text-transform:none;">{{ $invoice->currency_symbol }} {{ number_format($invoice->grand_total, 2) }}</strong></td>
              <td align="right" style="color:#7c2d12;font-size:12px;text-transform:uppercase;">Status<br><strong style="color:#ea580c;font-size:15px;text-transform:uppercase;">UNPAID</strong></td>
            </tr>
          </table>
        </td>
      </tr>
      ...
      <tr>
        <td style="padding:32px 40px 16px;">
          ...
          <p style="margin:0 0 16px;color:#4b5563;font-size:14px;line-height:1.7;">This is a friendly follow-up reminder regarding outstanding **Invoice {{ $invoice->invoice_number }}** issued on **{{ $invoice->invoice_date->format('d M Y') }}**. According to our records, the payment is due on or before **{{ $invoice->due_date->format('d M Y') }}** and has not been settled yet.</p>
          <p style="margin:0 0 24px;color:#4b5563;font-size:14px;line-height:1.7;">We have **attached a copy of the invoice as a PDF** to this email for your convenience. Below is a summary of the outstanding items:</p>
      ...
      <tr>
        <td style="padding:0 40px 24px;">
          <table width="100%" cellpadding="0" cellspacing="0" style="border-radius:8px;overflow:hidden;border:1px solid #e2e8f0;">
            ...
            @endforeach
            <tr style="background:#fff7ed;"><td colspan="2" align="right" style="padding:12px 16px;color:#7c2d12;font-size:13px;font-weight:700;">Outstanding Balance</td><td align="right" style="padding:12px 16px;color:#ea580c;font-size:16px;font-weight:800;">{{ $invoice->currency_symbol }} {{ number_format($invoice->grand_total, 2) }}</td></tr>
          </table>
        </td>
      </tr>
      ...
      <tr>
        <td style="padding:0 40px 32px;">
          <p style="margin:0 0 16px;color:#4b5563;font-size:14px;line-height:1.7;">If you have already processed this payment, please disregard this email. Otherwise, kindly remit the outstanding balance at your earliest convenience.</p>
          <p style="margin:0;color:#4b5563;font-size:14px;">Thank you for your business and cooperation.</p>
      ...
      <tr>
        <td style="background:#f8fafc;padding:24px 40px;text-align:center;border-top:1px solid #e2e8f0;">
          <p style="margin:0;color:#9ca3af;font-size:11px;">Follow-up generated automatically by <strong>{{ config('app.name') }}</strong> · {{ config('app.url') }}</p>
        </td>
      </tr>
    </table>
  </td></tr>
