...
@section('header_title', 'Modify Expense Entry')
...
<div class="container-fluid p-0 d-flex flex-column gap-4" style="max-width: 800px;">
    ...
    <div class="premium-card">
        <div class="mb-4">
            ...
            <p class="text-muted fs-7 m-0">Edit invoice details, receipt document attachments, or GST records.</p>
        </div>

        <form method="POST" action="{{ route('expenses.update', $expense->id) }}" enctype="multipart/form-data" id="expense-form">
            @csrf
            @method('PUT')
            ...
            <div class="row g-3">
                ...
                <div class="col-12 col-md-6">
                    ...
                    <input type="date" name="expense_date" class="form-control rounded-3 @error('expense_date') is-invalid @enderror" value="{{ old('expense_date', $expense->expense_date->format('Y-m-d')) }}" required>
                    @error('expense_date')
                ...
                <div class="col-12 col-md-6">
                    ...
                    <input type="text" name="vendor_name" class="form-control rounded-3 @error('vendor_name') is-invalid @enderror" value="{{ old('vendor_name', $expense->vendor_name) }}" required>
                    @error('vendor_name')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-12 col-md-6">
                    ...
                    <input type="text" name="vendor_gstin" class="form-control rounded-3 @error('vendor_gstin') is-invalid @enderror" value="{{ old('vendor_gstin', $expense->vendor_gstin) }}" placeholder="e.g. 27AAAAA1111A1Z1" style="text-transform: uppercase;">
                    @error('vendor_gstin')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <!-- Category -->
                @php
                    $presets = ['Software & Subscriptions', 'Domain & Hosting', 'Utilities & Internet', 'Office Supplies', 'Rent & Workspace', 'Marketing & Advertising'];
                    $isCustom = !in_array($expense->category, $presets);
                @endphp
                <div class="col-12 col-md-6">
                    ...
                    <select name="category_select" id="category_select" class="form-select rounded-3" required>
                        <option value="Software & Subscriptions" {{ old('category_select', $expense->category) === 'Software & Subscriptions' ? 'selected' : '' }}>Software & Subscriptions</option>
                        <option value="Domain & Hosting" {{ old('category_select', $expense->category) === 'Domain & Hosting' ? 'selected' : '' }}>Domain & Hosting</option>
                        <option value="Utilities & Internet" {{ old('category_select', $expense->category) === 'Utilities & Internet' ? 'selected' : '' }}>Utilities & Internet</option>
                        <option value="Office Supplies" {{ old('category_select', $expense->category) === 'Office Supplies' ? 'selected' : '' }}>Office Supplies</option>
                        <option value="Rent & Workspace" {{ old('category_select', $expense->category) === 'Rent & Workspace' ? 'selected' : '' }}>Rent & Workspace</option>
                        <option value="Marketing & Advertising" {{ old('category_select', $expense->category) === 'Marketing & Advertising' ? 'selected' : '' }}>Marketing & Advertising</option>
                        <option value="custom" {{ $isCustom ? 'selected' : '' }}>Other / Custom (Specify...)</option>
                    </select>
                    <input type="hidden" name="category" id="category_hidden" value="{{ old('category', $expense->category) }}">
                </div>

                <!-- Custom Category Input -->
                <div class="col-12 {{ $isCustom ? '' : 'd-none' }}" id="custom-category-group">
                    <label class="form-label fw-semibold text-muted fs-7">Specify Custom Category <span class="text-danger">*</span></label>
                    <input type="text" id="custom_category_input" class="form-control rounded-3" value="{{ $isCustom ? $expense->category : '' }}" placeholder="Enter custom category name">
                </div>
                ...
                <div class="col-12 col-md-6">
                    ...
                    <div class="input-group">
                        ...
                        <input type="number" step="0.01" min="0" name="base_amount" id="base_amount" class="form-control rounded-end-3 @error('base_amount') is-invalid @enderror" value="{{ old('base_amount', $expense->base_amount) }}" required>
                    </div>
                    @error('base_amount')
                ...
                <div class="col-12 col-md-6">
                    ...
                    <select id="gst_type" class="form-select rounded-3">
                        ...
                        <option value="local_18">Local GST 18% (9% CGST + 9% SGST)</option>
                        ...
                        <option value="custom" selected>Custom / Existing Tax Configuration</option>
                    </select>
                </div>
                ...
                <div class="col-12 col-md-4">
                    ...
                    <div class="input-group">
                        <span class="input-group-text rounded-start-3">₹</span>
                        <input type="number" step="0.01" min="0" name="cgst" id="cgst" class="form-control rounded-end-3" value="{{ old('cgst', $expense->cgst) }}">
                    </div>
                </div>
                ...
                <div class="col-12 col-md-4">
                    ...
                    <div class="input-group">
                        <span class="input-group-text rounded-start-3">₹</span>
                        <input type="number" step="0.01" min="0" name="sgst" id="sgst" class="form-control rounded-end-3" value="{{ old('sgst', $expense->sgst) }}">
                    </div>
                </div>
                ...
                <div class="col-12 col-md-4">
                    ...
                    <div class="input-group">
                        <span class="input-group-text rounded-start-3">₹</span>
                        <input type="number" step="0.01" min="0" name="igst" id="igst" class="form-control rounded-end-3" value="{{ old('igst', $expense->igst) }}">
                    </div>
                </div>
                ...
                <div class="col-12 col-md-6">
                    ...
                    <div class="input-group">
                        <span class="input-group-text rounded-start-3">₹</span>
                        <input type="number" step="0.01" min="0" name="total_amount" id="total_amount" class="form-control rounded-end-3 bg-light @error('total_amount') is-invalid @enderror" value="{{ old('total_amount', $expense->total_amount) }}" readonly required>
                    </div>
                    @error('total_amount')
                        <div class="invalid-feedback d-block">{{ $message }}</div>
                    @enderror
                ...
                <div class="col-12 col-md-6">
                    ...
                    <select name="payment_mode" class="form-select rounded-3">
                        <option value="Current Account Debit Card" {{ old('payment_mode', $expense->payment_mode) === 'Current Account Debit Card' ? 'selected' : '' }}>Current Account Debit Card</option>
                        <option value="Net Banking" {{ old('payment_mode', $expense->payment_mode) === 'Net Banking' ? 'selected' : '' }}>Net Banking</option>
                        <option value="Credit Card" {{ old('payment_mode', $expense->payment_mode) === 'Credit Card' ? 'selected' : '' }}>Credit Card</option>
                        <option value="UPI / GPay / PhonePe" {{ old('payment_mode', $expense->payment_mode) === 'UPI / GPay / PhonePe' ? 'selected' : '' }}>UPI / GPay / PhonePe</option>
                ...
                <!-- Receipt Upload -->
                <div class="col-12">
                    <label class="form-label fw-semibold text-muted fs-7">Replace Receipt File (Optional, max 5MB)</label>
                    ...
                    @if($expense->receipt_url)
                        <div class="mt-2 fs-7 d-flex align-items-center gap-2">
                            ...
                            <a href="{{ Storage::url($expense->receipt_url) }}" target="_blank" class="fw-semibold text-primary text-decoration-none">Download / View Document</a>
                        </div>
                    @endif
                    @error('receipt')
                        <div class="invalid-feedback">{{ $message }}</div>
                ...
                <div class="col-12 mt-4">
                    <div class="form-check form-switch p-3 bg-light rounded-3 border border-slate-100 d-flex justify-content-between align-items-center">
                        ...
                        <input class="form-check-input ms-0" type="checkbox" role="switch" name="is_itc_eligible" id="is_itc_eligible" value="1" {{ old('is_itc_eligible', $expense->is_itc_eligible) ? 'checked' : '' }} style="width: 48px; height: 24px; cursor: pointer;">
                    </div>
                </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        gstTypeSelect.addEventListener('change', function() {
            if (this.value === 'custom') {
                // Don't modify cgst/sgst/igst inputs if custom is loaded initially or selected
            } else {
                calculateTaxes();
            }
        });
        ...
        // Run category checks on load
        if (categorySelect.value === 'custom') {
            customCategoryGroup.classList.remove('d-none');
            customCategoryInput.setAttribute('required', 'required');
        }

        // Initial calculate total based on prefilled values
        calculateTaxes();
    });
</script>
@endsection
